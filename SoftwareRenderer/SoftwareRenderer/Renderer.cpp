...
//有参构造
Renderer::Renderer(HDC hdc,int devicewidth,int deviceheight,std::unique_ptr<Camera> &cam)
{
	this->screen = hdc;
...
//清空缓冲区
void Renderer::Zbuffer_Clear(std::unique_ptr<DepthBuffer> &depthbuffer)
{
...
//绘制网格
void Renderer::DrawMesh(std::unique_ptr<Mesh> &m, Shader* shader, std::unique_ptr<DepthBuffer> &depthbuffer)
{
...
//获取f
void Renderer::DrawByIndex(std::unique_ptr<Mesh> &m, Shader* shader, std::unique_ptr<DepthBuffer>& depthbuffer)
{
...
//作用：存疑,直接读取三个顶点
void Renderer::DrawByArray(std::unique_ptr<Mesh> &m, Shader* shader, std::unique_ptr<DepthBuffer>& depthbuffer)
{
...
//绘制图元
void Renderer::DrawPrimitive(Vertex v0, Vertex v1, Vertex v2, Shader* shader, std::unique_ptr<DepthBuffer>& buffer)
{
	//做顶点着色
...
//光栅化三角形
void Renderer::RasterizeTrangle(Vertex v0, Vertex v1, Vertex v2, Shader* shader, std::unique_ptr<DepthBuffer>& depthbuffer)
{
...
//绘制三维底三角形
void Renderer::DrawBottomTrangle3D(Vertex v0, Vertex v1, Vertex v2, Vertex v3, Shader* shader, std::unique_ptr<DepthBuffer>& depthbuffer, int swapIndex)
{
...
//绘制三维顶三角形
void Renderer::DrawTopTrangle3D(Vertex v0, Vertex v1, Vertex v2, Vertex v3, Shader* shader, std::unique_ptr<DepthBuffer>& depthbuffer, int swapIndex)
{
	float x0 = v0.Position.x;
	float y0 = v0.Position.y;
	float x1 = v1.Position.x;
...
//绘制三维直线(y值永远相等的线)
void Renderer::DrawLine3D(Vertex v0, Vertex v1, Shader* shader, Vector3f p0, Vector3f p1, Vector3f p2, std::unique_ptr<DepthBuffer>& depthbuffer)
{
	float x0 = v0.Position.x;
	float y0 = v0.Position.y;
	float x1 = v1.Position.x;
...
//准备光栅化,将顶点坐标对齐到像素中点
inline void Renderer::PrepareRasterization(Vertex &vertex,std::unique_ptr<DepthBuffer> &buffer)
{
	//透视除法,对齐到归一化设备坐标
...
//z深度测试
bool Renderer::DrawZTestAndWrite(int x,int y,float depth, std::unique_ptr<DepthBuffer>& zbuffer)
{
