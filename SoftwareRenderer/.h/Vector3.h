...
public:
	...
	Vector3<T>()
	{
	 ...
	 Vector3<T> operator + (const Vector3<T>& right) const;             //向量的加法
	 Vector3<T> operator - (const Vector3<T>& right) const;             //向量的减法
	 Vector3<T> operator * (float value) const;                         //向量的乘法
	 Vector3<T> operator / (float value) const;                         //向量的除法
	 float SquareMagnitude();                                           //计算两个向量dot
	 float Magnitude();                                                 //模长开根号
	 Vector3<T>& Normalize();                                           //单位化

	float static Dot(const Vector3<T>& left, const Vector3<T>& right);      //点乘（v1 dot v2)
	float  Dot(const Vector3<T>& right);                                    //点乘（v1.dot(v2)),作用一样的
	Vector3<T> static Cross(const Vector3<T>& left, const Vector3<T>& right);   //叉乘
    Vector3<T> static Lerp(const Vector3<T>& left, const Vector3<T>& right, float t);     //插值
